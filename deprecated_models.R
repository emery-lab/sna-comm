...
one.factor.model = function(matrix, env, factor, presence = FALSE){
  ...
  if(presence == FALSE){
    x = lmer(abundance ~  veg_long[, ind] + (veg_long[,ind]|species) + (1|node),data = veg_long)
  } else if (presence == TRUE){
    x = glmer(abundance ~  veg_long[, ind] + (1|species) + (1|node), family = binomial,data = veg_long)
  }
  
  
  
  if(isSingular(x)){
    x = lmer(abundance ~  veg_long[, ind] + (veg_long[, ind]-1|species),data = veg_long)
  }
...
abundance.daily = run.models(matrix, env1, presence = FALSE)
abundance.seaso = run.models(matrix, env3, presence = FALSE)
...
add_distributions = function(model, term){
  ...
  #tt = as.data.frame(cbind(x, reff$species$`veg_long[, ind]`))
  #tt = tt[order(tt$V2),]
...
add_species = function(model, term){
  ...
  print(x)
  #return(x)
  species = c("SAXRHO", "ALLGEY", "LUZSPI", "TETGRA", "CERARV", "LEWPYG")
  ...
  if(isSingular(model)){
    ...
    for(i in 1:6){
      ...
      points(xx$mean, shift[which(names %in% term)] + 0.05 * i, pch = 22, bg = cols[i], col = "black", cex = 1.2)
    }
  }
...
png("output/figures/FigureXX_1.png", res = 200, width = 6, height = 6, units = "in")
plot.slopes(presence.daily, presence = TRUE)
legend(1.5, 6.5, pch = 22, col = "black", legend = c(rev(c("SAXRHO", "ALLGEY", "LUZSPI", "TETGRA", "CERARV", "LEWPYG"))), pt.bg = rev(cols), bty = "n")
dev.off()

png("output/figures/FigureXX_2.png", res = 200, width = 6, height = 6, units = "in")
plot.slopes(presence.seaso, presence = TRUE)
#legend(1.5, 6.5, pch = 21, col = "black", legend = c(species_gs), pt.bg = cols, bty = "n")
dev.off()

png("output/figures/FigureXX_3.png", res = 200, width = 6, height = 6, units = "in")
plot.slopes(abundance.daily)
#legend(1.5, 6.5, pch = 21, col = "black", legend = c(species_gs), pt.bg = cols, bty = "n")
dev.off()

png("output/figures/FigureXX_4.png", res = 200, width = 6, height = 6, units = "in")
plot.slopes(abundance.seaso)
#legend(1.5, 6.5, pch = 21, col = "black", legend = c(species_gs), pt.bg = cols, bty = "n")
dev.off()
...
png("/Users/Will/Desktop/Fig2a.png", height = 5, width = 4.25, res =150, units = "in")
par(mar = c(3,3,3,3), cex.axis = 2)
barplot(count_sig(presence.daily), ylim = c(0,40), beside = T, space = 0, col = cols)
box(bty = "l", lwd = 2)
dev.off()

png("/Users/Will/Desktop/Fig2b.png", height = 5, width = 4.25, res =150, units = "in")
par(mar = c(3,3,3,3), cex.axis = 2)
barplot(count_sig(abundance.daily), ylim = c(0,40), beside = T, space = 0, col = cols)
box(bty = "l", lwd = 2)
dev.off()

png("/Users/Will/Desktop/Fig2c.png", height = 5, width = 4.25, res =150, units = "in")
par(mar = c(3,3,3,3), cex.axis = 2)
barplot(count_sig(presence.seaso), ylim = c(0,40), beside = T, space = 0, col = cols)
box(bty = "l", lwd = 2)
dev.off()

png("/Users/Will/Desktop/Fig2d.png", height = 5, width = 4.25, res =150, units = "in")
par(mar = c(3,3,3,3), cex.axis = 2)
...
# SAXRHO, ALLGEY, LUZSPI, TETGRA, CERARV, LEWPYG
